...
describe('Sudoku Solver', function() {
	...
	'706000810\n' +
	'300090000';
    var parsedBoard, expectedPositions;
	...
	it('should check that each value in a 3x3 square does not match the input', function() {
	    // No match. Return true
	    expect(solver.checkSquare(parsedBoard, 2, 2, 1)).to.be.ok;
	    expect(solver.checkSquare(parsedBoard, 7, 7, 9)).to.be.ok;
	    // Match found. Return false
	    expect(solver.checkSquare(parsedBoard, 2, 2, 9)).to.not.be.ok;
	    expect(solver.checkSquare(parsedBoard, 7, 7, 1)).to.not.be.ok;
	});
    });
